...
def list_mp4(path):
    ...
    if os.path.isdir(path):
        mp4s = []
        for mp4 in os.listdir(path):
            if mp4.endswith('.mp4'):
                mp4s.append(mp4)
        return mp4s

def usage():
    print 'python mp4_join.py --output target.mp4 [1.mp4 [2.mp4 ...]] '
    print 'python mp4_join.py --output target.mp4 [path] '
...
def main():
    ...
    if len(args) == 0:
        concat_mp4s(list_mp4('./'), output)
    elif len(args) == 1:
        concat_mp4s(list_mp4(args[0]), output)
    else:
